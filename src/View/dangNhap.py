...
# ... (Toàn bộ code Class LoginPage ở trên giữ nguyên KHÔNG SỬA GÌ CẢ) ...

# --- [ĐOẠN CODE TEST GIAO DIỆN KHÔNG CẦN SỬA LOGIC] ---
if __name__ == "__main__":
    import sys
    import os
    import customtkinter as ctk

    # 1. Lấy vị trí file hiện tại (dangnhap.py)
    current_path = os.path.dirname(os.path.abspath(__file__))

    # 2. Xác định thư mục gốc dự án (Lùi 2 cấp: View -> src -> Root)
    project_root = os.path.dirname(os.path.dirname(current_path))

    # 3. [MẤU CHỐT] Ép chương trình "nhảy" về thư mục gốc ngồi
    # Khi này, đường dẫn 'src/images/...' trong code cũ của bạn sẽ lại đúng!
    ...
    root.title("Test Giao Diện (Chế độ Root)")


    # 5. Hàm giả
    def test_login_success(name, uid, role):
        print(f"Login OK: {name} - {role}")
    ...
    # 6. Khởi tạo
    app_test = LoginPage(parent=root,
                         on_login_success=test_login_success,
                         on_show_forgot_pass=test_forgot_pass)


    # Mock Controller để không lỗi DB
    class MockController:
        def xu_ly_dang_nhap(self, u, p):
            return {"status": True, "data": {"id_nhan_vien": 1, "ho_ten": "Test", "role_name": "Admin"}}
